...
describe('env create scratch NUTs', () => {
  ...
  describe('flag failures', () => {
    it('non-existent config file', () => {
      execCmd('org:create:scratch -f badfile.json', { ensureExitCode: 'nonZero' });
    });
    it('wait zero', () => {
      execCmd('org:create:scratch -f config/project-scratch-def.json --wait 0', { ensureExitCode: 'nonZero' });
    });
    it('no edition or config', () => {
      execCmd('org:create:scratch', { ensureExitCode: 'nonZero' });
    });
    it('days out of bounds', () => {
      execCmd('org:create:scratch -f config/project-scratch-def.json -d 50', { ensureExitCode: 'nonZero' });
    });
  ...
  describe('successes', () => {
    ...
    it('creates an org from config file flag only', () => {
      const resp = execCmd<ScratchCreateResponse>(
        'org:create:scratch -f config/project-scratch-def.json --json  --wait 60',
        {
          ensureExitCode: 0,
        }
      ).jsonOutput?.result;
    ...
    it('creates an org from config file with "override" flags and custom admin email', () => {
      ...
      const resp = execCmd<ScratchCreateResponse>(
        `org:create:scratch -f config/project-scratch-def.json --json --username ${expectedUsername} --description "new one" --name TheOrg --wait 60 --admin-email shane@mailinator.com`,
        {
          ensureExitCode: 0,
        }
      ).jsonOutput?.result;
    ...
    it('stores default in local sf config', async () => {
      const resp = execCmd<ScratchCreateResponse>(
        'org:create:scratch --edition developer --json --set-default  --wait 60',
        {
          ensureExitCode: 0,
        }
      ).jsonOutput?.result;
